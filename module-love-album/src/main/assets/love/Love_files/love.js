(function(window){
    ...
    Seed.prototype = {
        ...
        canMove: function() {
            return this.cirle.point.y < (this.tree.height + 20/2);
        },
        move: function(x, y) {
            this.clear();
            this.drawCirle();
        ...
        drawText: function() {
            ...
            ctx.scale(scale, scale);
            ctx.moveTo(0, 0);
    	    ctx.lineTo(15/2+1, 15/2+1);
    	    ctx.lineTo(60+5, 15/2+1);
            ctx.stroke();

            ctx.moveTo(0, 0);
            ctx.scale(0.75, 0.75);
            ctx.font = "8px 微软雅黑,Verdana"; // 字号肿么没有用? (ˉ(∞)ˉ)
            ctx.fillText("点这里⁄(⁄ ⁄•⁄ω⁄•⁄ ⁄)⁄", 23/2, 16/2);
            ctx.restore();
        },
        clear: function() {
            var ctx = this.tree.ctx, cirle = this.cirle;
    ...
    Tree.prototype = {
        ...
        createBloom: function(width, height, radius, figure, color, alpha, angle, scale, place, speed) {
            var x, y;
            while (true) {
                x = random(20/2, width - 20/2);
                y = random(20/2, height - 20/2);
                if (inheart(x - width / 2, height - (height - 40/2) / 2 - y, radius)) {
        ...
        jump: function() {
            ...
            if ((blooms.length && blooms.length < 3) || !blooms.length) {
                ...
                for (var i = 0; i < random(1,2); i++) {
                    blooms.push(this.createBloom(width / 2 + width, height, r, figure, null, 1, null, 1, new Point(random(-100/2,600/2), 720/2), random(200/2,300/2)));
                }
            }
        }
    }
    ...
    Bloom.prototype = {
        ...
        jump: function() {
            ...
            if (s.point.x < -20/2 || s.point.y > height + 20/2) {
                s.tree.removeBloom(s);
            } else {
                s.draw();
